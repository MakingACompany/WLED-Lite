...
**Open issues / things I deferred (later addressed — see below):**

- ~~**Server-side hard-block on `/` until PIN is set.**~~ **Done.** `wled_server.cpp:612` `/` handler now sets `forceWelcome = true` when `strlen(settingsPIN) == 0`, gated by `-D WLED_LITE_SECURITY_GATES`. End user never sees this — by the time they receive the device the PIN is set. Note: `?sliders` still bypasses to the user UI even without a PIN (trial mode in AP without WiFi yet); `/settings/*` is still reachable without a PIN until the maintainer sets one (it has to be — that's where the PIN form lives). The bigger commercial-class lock-down — refusing `/settings` entirely except for the PIN-set sub-page until a PIN exists — is still open.
- ~~**`/reset` is unauthenticated** in upstream.~~ **Done.** `/reset` now refuses unless `correctPIN` is true (factory-state device with no PIN is still allowed to reboot, since there's no auth available to require yet). Same `-D WLED_LITE_SECURITY_GATES` build flag.
- **Still open: `/settings/*` lockdown until PIN is set.** Today the maintainer can still hit any settings sub-page without a PIN, even pre-onboarding. For a commercial production line the right behavior is: until a PIN exists, allow only `/settings/wifi` and `/settings/sec`; everything else 401s back to `/welcome`. This is a bigger surgery on `serveSettings()` and warrants its own pass.
- **WebSocket /ws auth.** Same shape as `/json` POST: state mutations OK without PIN, config mutations require PIN. Both transports inherit the upstream auth model unchanged. Re-verify when the slim UI's WS path is exercised in production.

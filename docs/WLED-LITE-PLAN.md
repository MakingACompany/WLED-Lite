...
| 10 | Multi-controller sync (ESP-NOW + WiFi) | See [`docs/multi-controller-sync.md`](multi-controller-sync.md). Both transports kept and fully functional. UDP-on-port-21324 and ESP-NOW share the same packet protocol and 8-bit `syncGroups` bitmask routing. WLED-Lite-specific: a single one-bit default flipped via `-D WLED_LITE_SYNC_DEFAULTS` in `wled.h` — `sendNotifications` from `false → true` so two devices on the same WiFi with the (already-default) group 0x01 mirror automatically out of the box. ESP-NOW stays opt-in per device (security + power). `settings_sync.htm` left as-is; pairing-wizard UI work deferred to Task #5 Phase B. Flash unchanged. |

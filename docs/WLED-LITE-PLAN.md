...
| 5 | Design role-based UI split | See [`docs/ui-split-design.md`](ui-split-design.md). Auth model: reuse upstream's `settingsPIN`. Default PIN: maintainer sets during AP-mode onboarding. UI impl: slim `index.htm` (color/effect/brightness/on-off/schedule); `/settings/*` stays as admin surface. **Phases A, B, C, E done**: effect curation (Task #6), slim user UI with "Lantern" aesthetic (~6 KB gzipped, ~25 KB smaller than upstream), welcome wizard with 3-step PIN flow, server-side gates (`/` forces welcome until PIN set; `/reset` requires PIN). Open: Phase D (settings sub-page tone audit) + full `/settings/*` lockdown until PIN exists (commercial-class enforcement). Design commit `56df24d0`; slim-UI impl `eb3e74c1`; security gates `aaf3719c`. |
...
| 11 | Document upstream-sync workflow | See [`docs/upstream-sync.md`](upstream-sync.md). Playbook for monthly merges from `wled/WLED`: setup, branch-per-merge (`sync/upstream-YYYY-MM-DD`), per-file conflict guide (covers `platformio.ini`, `package.json`, `wled.h`, `button.cpp`, `FX.cpp`, etc.), what NOT to re-merge (re-introducing ESP8266 envs, dropped feature flags), post-merge audit + bench-test checklist, tagged-release strategy, OTA release-name caveats. Captures the current divergence: 20 ahead / 16 behind upstream `42f4bcb8` since fork point `8e94cf5b`. No code changes. Commit `311205af`. |
| — | **Bench-test wrap-up** | Three WLED-Lite size variants now live: `xiao_esp32s3_plus` (16 MB OPI), `xiao_esp32s3_plus_8MB` (8 MB OPI), `xiao_esp32s3_plus_4M` (4 MB QSPI). All three in `default_envs`, all three with committed `firmware/` bins. WLED-Lite build flags factored into `[wled_lite_common]` shared section. CI workflow `.github/workflows/wled-lite-build.yml` builds all three on push. `/settings/*` lockdown lands (only WiFi + Security reachable when no PIN). Phase D tone audit doc, `docs/flash-from-windows.md` quick-start, `docs/phase-d-tone-audit.md`, `tools/post-merge-check.sh` (37 invariant checks). Maintainer can pull and flash from Windows tonight — see the flash-from-windows doc. |
| 12 | **Hardware bring-up** — XIAO Plus boot loop diagnosed | Root cause: Seeed's stock `seeed_xiao_esp32s3` board JSON has `variant=XIAO_ESP32S3` + extra_flags that boot-loop on real XIAO Plus rev 0.2 hardware. Fixed by introducing a custom board JSON (`boards/xiao_esp32s3_plus_n16r8v.json`) that extends `esp32-s3-devkitc-1`'s variant while declaring accurate flash/PSRAM specs. Secondary find: `esptool merge_bin --flash-mode dio` (not `qio`) is required because Puya flash can't QIO before the second-stage bootloader initializes it. Validated by piecemeal flash booting clean while the consolidated `merged-flash.bin` (built with qio) bootlooped. |
| 13 | Daily on/off schedule | See [`docs/scheduling.md`](scheduling.md). End user sets schedule from slim UI (PIN-less) via `POST /wled-lite/schedule`. Endpoint writes preset 250 ("Daily ON" = current state) + preset 249 ("Daily OFF") and two timer entries (weekdays 0xFF). NTP defaults: `WLED_NTP_ENABLED=true`, `NTP_SYNC_INTERVAL=300UL`, `WLED_LITE_NTP_SERVER="time.nist.gov"` — schedule fires accurately out of the box on any internet-connected install. Slim UI offers `Never / 1hr / 4hr / 12hr` auto-shutoff in a separate card (nightlight wrapper). Persistence via cfg.json + presets.json. |
| 14 | Admin segment editor + user-facing pill selector | See [`docs/segments.md`](segments.md). New admin page at `/wled-lite/segments` (PIN-gated): table-style {name, start, stop} editor that POSTs back through `/json/state`. Boot preset 248 written on save so geometry survives reboot independent of cfg-save triggers. Slim UI grows a Section card with `[All] [seg1] [seg2]…` pills when ≥ 2 segments exist; `routedPayload()` JS helper rewrites the `seg` field of all mutations to target the active pill. Build flag `WLED_LITE_SEGMENTS`. |
| 15 | Segment pill polish (subscripts + nested drawer) | Trailing digits in segment names render as visual subscript (`B1` → **B₁**); underlying name unchanged so admin / state-API references stay literal. For 5+ segments, the pill row collapses to `[All] [Letters ▸]` with a drawer of individual pills underneath — tap Letters to expand, All to collapse + reset. Pure UI work; no backend changes. Commit `824e30d3`. |
| — | **NTP-only decision** | No manual time-entry UI / offline RTC fallback. Smart-outlet users cut all power (controller has no clock to keep anyway); power-outage scenarios that take the controller down likely also take the router down. The narrow remaining sliver (controller up, router up, NTP unreachable) is too small to justify the code. `time.nist.gov` is the hard-wired default via `WLED_LITE_NTP_SERVER`. |
| 16 | Reference guides — user.html + admin.html | New `docs/guide/` static-HTML reference set: `user.html` (customer-facing, scrollable walkthrough with annotated phone mockups + live iro.js color wheel demo) and `admin.html` (maintainer reference: every settings page, every WLED-Lite custom flag, pinout, flashing procedures, maintenance playbook). Both self-contained for hosting on the installer's website. Shared `styles.css` `@imports` a copy of the slim UI's CSS so the mockups stay visually true to the firmware. Companion markdown manual at [`docs/user-manual.md`](user-manual.md). |

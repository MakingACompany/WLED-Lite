...
| 7 | Support 8 strips on configurable pins | See [`docs/hardware-pinmap.md`](hardware-pinmap.md). S3 has 4 RMT + 8 I2S/LCD channels (`WLED_MAX_DIGITAL_CHANNELS` = 12) — 8 simultaneous LED outputs is well within capability. Pin map for XIAO ESP32-S3 Plus: LED buses on GPIO 4, 7, 8, 9, 10, 11, 12, 13 (4 front + 4 Plus sub-header); I2C on GPIO 5/6; buttons on GPIO 1/2; HW UART 43/44 kept free for serial-adapter debug; GPIO 38–42 reserved for expansion. Build-flag defaults applied via `DATA_PINS`, `LED_TYPES`, `PIXEL_COUNTS`, `BTNPIN`, `HW_PIN_SDA`, `HW_PIN_SCL`; admin UI (`/settings/leds`, `/settings/pin`) handles per-device override. |

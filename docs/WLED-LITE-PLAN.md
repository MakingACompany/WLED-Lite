...
| 5 | Design role-based UI split | See [`docs/ui-split-design.md`](ui-split-design.md). Auth model: reuse upstream's `settingsPIN`. Default PIN: maintainer sets during AP-mode onboarding. UI impl: slim `index.htm` (color/effect/brightness/on-off/schedule); `/settings/*` stays as admin surface. **Phases A, B, C, E done**: effect curation (Task #6), slim user UI with "Lantern" aesthetic (warm amber on amber-black, mobile-first, ~6 KB gzipped in flash, ~25 KB smaller than upstream's index), welcome wizard with 3-step PIN flow. Open: Phase D (settings sub-page language audit) + server-side hard-block on `/` until PIN is set (commercial-deployment concern). Design commit `56df24d0`; implementation commit pending. |
